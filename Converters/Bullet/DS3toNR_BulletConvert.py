...
def convert(source_csv: Path, target_csv: Path, output_csv: Path) -> None:
    ...
    for _, srow in source_df.iterrows():
        ...
        for tcol in target_columns:
            ...
            # Keep target/template values for pad/raw fields
            if col_key.startswith("pad") or "pad" in col_key:
                continue
...
def main():
    ...
    parser.add_argument(
        '--source',
        default='Bullet_midir.csv',
        help='Path to the DS3 source CSV (default: Bullet_midir.csv)',
    )
    parser.add_argument(
        '--target',
        default='Bullet_caligo.csv',
        help='Path to the Nightreign target/template CSV (default: Bullet_caligo.csv)',
    )
    parser.add_argument(
        '--output',
        default='Bullet_midir_DS3toNR.csv',
        help='Path to write the converted CSV (default: Bullet_midir_DS3toNR.csv)',
    )
