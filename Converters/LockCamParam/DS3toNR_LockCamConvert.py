...
def main():
    parser = argparse.ArgumentParser(
        description='Convert DS3 LockCamParam CSV rows to Nightreign-compatible schema using a Nightreign target/template CSV.'
    )
    parser.add_argument(
        '--source',
        default='LockCamParam_midir.csv',
        help='Path to the DS3 source CSV (default: LockCamParam_midir.csv)',
    )
    parser.add_argument(
        '--target',
        default='LockCamParam_caligo.csv',
        help='Path to the Nightreign target/template CSV (default: LockCamParam_caligo.csv)',
    )
    parser.add_argument(
        '--output',
        default='LockCamParam_midir_DS3toNR.csv',
        help='Path to write the converted CSV (default: LockCamParam_midir_DS3toNR.csv)',
    )
