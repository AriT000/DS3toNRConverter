...
def convert(source_csv: Path, target_csv: Path, output_csv: Path) -> None:
    ...
    for _, srow in source_df.iterrows():
        ...
        for tcol in target_columns:
            col_key = str(tcol).strip().lower()

            # Skip pad/reserved/raw fields (important for ThinkParam stability)
            if col_key.startswith("pad") or "reserve" in col_key:
                continue

            source_col = source_lookup.get(col_key)
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert DS3 NpcThinkParam CSV rows to Nightreign-compatible schema using a Nightreign target/template CSV.'
    )
    parser.add_argument(
        '--source',
        default='NpcThinkParam_midir.csv',
        help='Path to the DS3 source CSV (default: NpcThinkParam_midir.csv)',
    )
    parser.add_argument(
        '--target',
        default='NpcThinkParam_caligo.csv',
        help='Path to the Nightreign target/template CSV (default: NpcThinkParam_caligo.csv)',
    )
    parser.add_argument(
        '--output',
        default='NpcThinkParam_midir_DS3toNR.csv',
        help='Path to write the converted CSV (default: NpcThinkParam_midir_DS3toNR.csv)',
    )
