...
def main():
    parser = argparse.ArgumentParser(
        description='Convert DS3 NpcParam CSV rows to Nightreign-compatible schema using a Nightreign target/template CSV.'
    )
    parser.add_argument(
        '--source',
        default='NpcParam_midir.csv',
        help='Path to the DS3 source CSV (default: NpcParam_midir.csv)',
    )
    parser.add_argument(
        '--target',
        default='NpcParam_caligo.csv',
        help='Path to the Nightreign target/template CSV (default: NpcParam_caligo.csv)',
    )
    parser.add_argument(
        '--output',
        default='NpcParam_midir_DS3toNR.csv',
        help='Path to write the converted CSV (default: NpcParam_midir_DS3toNR.csv)',
    )
