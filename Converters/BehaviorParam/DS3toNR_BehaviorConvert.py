...
def main():
    parser = argparse.ArgumentParser(
        description='Convert DS3 BehaviorParam CSV rows to Nightreign-compatible schema using a Nightreign target/template CSV.'
    )
    parser.add_argument(
        '--source',
        default='BehaviorParam_midir.csv',
        help='Path to the DS3 source CSV (default: BehaviorParam_midir.csv)',
    )
    parser.add_argument(
        '--target',
        default='BehaviorParam_caligo.csv',
        help='Path to the Nightreign target/template CSV (default: BehaviorParam_caligo.csv)',
    )
    parser.add_argument(
        '--output',
        default='BehaviorParam_midir_DS3toNR.csv',
        help='Path to write the converted CSV (default: BehaviorParam_midir_DS3toNR.csv)',
    )
